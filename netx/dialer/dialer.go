...
func New(resolver modelx.DNSResolver, dialer modelx.Dialer) *dnsdialer.DNSDialer {
	return dnsdialer.NewDNSDialer(resolver, dialer)
}

// NewTLS creates a new modelx.TLSDialer
func NewTLS(dialer modelx.Dialer, config *tls.Config) *tlsdialer.TLSDialer {
	return tlsdialer.NewTLSDialer(dialer, config)
}
