...
type SubmitterSession interface {
	...
	NewSubmitter(ctx context.Context) (model.Submitter, error)
}
...
func NewSubmitter(ctx context.Context, config SubmitterConfig) (model.Submitter, error) {
	if !config.Enabled {
...
var _ model.Submitter = stubSubmitter{}

type realSubmitter struct {
	subm   model.Submitter
	logger model.Logger
}
